...
import { ActionButton, SliderArea } from "./style";
import "react-responsive-carousel/lib/styles/carousel.min.css"; // requires a loader
import { Carousel } from "react-responsive-carousel";
import { DataContext } from "contexts/DataContextContainer";

const ShowOpensea = ({ action = () => {}, kevinNft = [] }) => {
  ...
  const { selectedTokenId, setSelectedTokenId } = useContext(DataContext);
  console.log(selectedTokenId);

  useEffect(() => {
    setSelectedTokenId(kevinNft[0].token_id);
    console.log("tid ", kevinNft[0].token_id);
    console.log("tLength ", kevinNft.length - 1);
  }, []);
  ...
  return (
    <>
      ...
      <SliderArea className="m-t-5">
        <div className="flex1 card-slider">
          <Carousel
            ...
            showThumbs={false}
            showStatus={false}
            onChange={(i) => setSelectedTokenId(kevinNft[i].token_id)}
          >
              ...
              return (
                <div className="card-content" key={_i}>
                  <div className="w114 d-flex align-items-center">
                    <img
                      src={item.image_thumbnail_url || ItemTemp}
                      className="w120 br-4"
      ...
      <div className="lbl-cap1 m-t-15 f-bold">
        Now it’s time to <span className="f-yellow">level up</span>.
      </div>
      <div className="lbl-cap1 m-t-15 f-bold">
        Atomic NFTs are stored on Arweave’s permaweb and can never be removed.
        They also earn attention rewards in KOII tokens whenever they are
        viewed, anywhere on the internet.
      </div>

      <ActionButton onClick={action} className="m-t-30">
        Evolve my NFT
      </ActionButton>
      <div className="lbl-cap3 m-t-15">
