    ...
    return (
      <div className="mobile-sticky-area">
        <div className="viewer-ct">
          <div className="text-bold">{show_digit_number(totalViewCt)}</div>
          <div>
            <Image src={IconEyes} width="25" />
          </div>
        </div>
        <div className="viewer-ct">
          <div className="text-bold">{show_digit_number(totalKoiCt)}</div>
          <div>
    ...
    axios
      ...
      .then((res) => {
        ...
        if (data === 0) {
          ...
          let totalKoiCT = 0
          let d = new Date()
          let offset = d.getTime() - 3600*24*1000
          data.forEach((element, _i) => {
            totalViewCT = totalViewCT + element.totalViews
            let str_created_at = element.createdAt || "1609500000";
            let created_at = Number(str_created_at) * 1000;
            if(created_at > offset){
              totalKoiCT ++
            }
          ...
          setTotalViewCt(totalViewCT)
          setTotalKoiCt(totalKoiCT * 1000)
          // console.log({totalKoiCT})
    ...
    <MetaWrapper>
      ...
      <LeaderboardContainer>
        <div className="leaderboard">
          <StickyUploadArea show_alert={show_alert} isScrollMode={mobileScroll} defaultAction={action}/>
